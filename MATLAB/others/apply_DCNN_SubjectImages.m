...
for iSubject = 1 : length(param.SubjectInd)
    ...
    %% visualization RDM
    fig_1       = figure(1);
    rsa.fig.showRDMs(results_RDM, fig_1, 1, [], 0, 8/4)
    ...
    if param.SAVE_PDF == true
        ...
        print('-dpdf', param.PDF_RESOLUTION, [param.save_Path '\'  param.Subject '_RDM_AlexNet' date '.pdf'])
        winopen([param.save_Path '\'  param.Subject '_RDM_AlexNet' date '.pdf'])
    ...
    for iNoise = 1 : length(param.coherence_Values) % loop over coherence levels
        ...
        for iLayer = 1 : length(param.Layers)
            ...
            for iCategory = 1 : length(param.Category)
                ...
                h = plot(results_MDS.MDS_Y{iNoise, iLayer}(category_Ind : category_Ind + param.NumImages - 1,1), results_MDS.MDS_Y{iNoise, iLayer}(category_Ind : category_Ind + param.NumImages - 1, 2), 'o');
    ...
    if param.SAVE_PDF == true
        ...
        print('-dpdf', param.PDF_RESOLUTION, [param.save_Path '\'  param.Subject '_MDS_AlexNet' date '.pdf'])
        winopen([param.save_Path '\'  param.Subject '_MDS_AlexNet' date '.pdf'])
    ...
    for iNoise = 1 : length(param.coherence_Values) % loop over coherence levels
        ...
        for iLayer = 1 : length(param.Layers)
            ...
            for iCategory = 1 : length(param.Category)
                ...
                h = plot(results_LumCont.LumCont{iNoise, iLayer}(category_Ind : category_Ind + param.NumImages - 1,1), results_LumCont.LumCont{iNoise, iLayer}(category_Ind : category_Ind + param.NumImages - 1, 2), 'o');
...
%                 axis off
    ...
    if param.SAVE_PDF == true
        ...
        print('-dpdf', param.PDF_RESOLUTION, [param.save_Path '\'  param.Subject '_ContLum' date '.pdf'])
        winopen([param.save_Path '\'  param.Subject '_ContLum' date '.pdf'])

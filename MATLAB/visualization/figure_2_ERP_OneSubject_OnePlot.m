...
for iCatg = 1 : length(group_catg)
    ...
    time_vec    = sum(abs(param.window_stim)) + sum(abs(param.window_dec)) + abs(param.winndo_gap);
    h           = plot([mean_data_stim NaN * ones(1, param.winndo_gap) mean_data_dec]);
...
plot([1 1] * time_vec - param.window_dec(2), axis_ylim , ':k')
...
aX.XTick            = linspace(0, time_vec, 2*axis_xlim_spc);
% aX.XTickLabel       = [linspace(param.window_stim(1), param.window_stim(end), axis_xlim_spc) linspace(param.window_dec(1), param.window_dec(end), axis_xlim_spc)];

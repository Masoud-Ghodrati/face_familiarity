...
for iTrial_Type_Ind = 1 : length(trial_Type_Index)
    ...
    sub_data = all_Subject_Res(:, all_Subject_Res(end, :) == trial_Type_Index(iTrial_Type_Ind));
    this_Condition_Response_Array{iTrial_Type_Ind} = [sub_data];
...
for iTrial_Type_Ind = 1 : length(trial_Type_Index)
    ...
    for iCoherence_Steps = 1 : length(unique_Noise_Coherence)
        ...
        my_d = dprime_simple(mean(temp_Accuracy_Array), 1-mean(temp_Accuracy_Array), length(temp_Accuracy_Array), length(temp_Accuracy_Array));
        % calculte accuracy for each orientaion difference
        accuracy_Array(iTrial_Type_Ind, iCoherence_Steps, 1) = my_d ;
